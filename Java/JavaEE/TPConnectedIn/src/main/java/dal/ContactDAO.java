...
public class ContactDAO {
	...
	private static final String INSERT = "INSERT INTO contacts (nom, prenom, date_naissance, telephone, url_reseau, poste, specialite) VALUES (?,?,?,?,?,?,?)";
	private static final String UPDATE = "UPDATE contacts SET nom = ?, prenom = ?, date_naissance = ?, telephone = ?, url_reseau = ?, poste = ?, specialite = ? WHERE id = ?";
	...
	public void insert(Contact contact) {
		try (Connection cnx = ConnectionProvider.getConnection()) {
			...
			ps.setString(4, contact.getNumTelephone());
			ps.setString(5, contact.getUrlReseau());
			ps.setString(6, contact.getNomPoste());
	...
	private Contact convertResultSetToContact(ResultSet rs) throws SQLException {
		...
		contact.setNumTelephone(rs.getString("telephone"));
		contact.setUrlReseau(rs.getString("url_reseau"));
		contact.setNomPoste(rs.getString("poste"));
